...
export function OrderStatusDropdown({ order }: { order: Order }) {
  ...
  const formattedStatus = currentStatus.charAt(0).toUpperCase() + currentStatus.slice(1);
  ...
  return (
    <DropdownMenu>
        <DropdownMenuTrigger asChild>
            <Button
                variant="outline"
                className={cn("w-48 justify-between capitalize", statusColors[currentStatus])}
                disabled={isLoading}
            >
                {isLoading ? <Loader2 className="animate-spin" /> : formattedStatus}
